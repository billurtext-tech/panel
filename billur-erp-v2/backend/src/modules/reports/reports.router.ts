...
router.get('/export/payroll', requirePermission('reports.export'),
  ...
  try {
    ...
    res.setHeader('Content-Disposition',
      `attachment; filename="payroll-${periodStart}_${periodEnd}.xlsx"`);
    await wb.xlsx.write(res);
    res.end();
  } catch (e) { next(e); }
});
...
router.get('/export/quality', requirePermission('reports.export'),
  ...
  try {
    ...
    if (since || until) ws.addRow([`Davr: ${since || ''} — ${until || ''}`]);
    ws.addRow([]);
    const header = ws.addRow([
    ...
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition',
      `attachment; filename="quality-${new Date().toISOString().slice(0, 10)}.xlsx"`);
    await wb.xlsx.write(res);
    res.end();
  } catch (e) { next(e); }
});
