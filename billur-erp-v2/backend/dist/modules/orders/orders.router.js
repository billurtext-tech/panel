...
const orders_parsers_1 = require("./orders.parsers");
const router = (0, express_1.Router)();
router.use(auth_1.requireAuth);
router.get('/', (0, auth_1.requirePermission)('orders.read'), async (req, res, next) => {
    try {
        ...
        const type = (0, query_1.getOptionalQueryString)(req.query.type);
        const params = [];
        const conds = [`o.deleted_at IS NULL`];
        if (client_id) {
            params.push(client_id);
            conds.push(`o.client_id = $${params.length}`);
        }
        if (status) {
            params.push(status);
...
router.post('/', (0, auth_1.requirePermission)('orders.create'), async (req, res, next) => {
    try {
        ...
        if (!order_type || !['speka', 'set', 'standard'].includes(order_type)) {
            throw (0, types_1.BadRequest)('order_type: speka/set/standard');
        }
        if (!client_id)
            throw (0, types_1.BadRequest)('Klient tanlang');
    ...
    `, [order_type, external_code || null, client_id, deadline || null,
        ...
        if (Array.isArray(items)) {
            for (const it of items) {
                if (!it.model_id || !it.color_id || !it.size_id || !it.ordered_qty)
                    continue;
        ...
        `, [ord.rows[0].id, it.model_id, it.color_id, it.size_id, it.ordered_qty, it.unit_price_uzs || null]);
            ...
            await pool_1.pool.query(`UPDATE orders SET total_pieces = $1 WHERE id = $2`, [totalPieces, ord.rows[0].id]);
        }
        ...
        await (0, security_1.auditLog)({
            ...
            after_value: ord.rows[0], ip_address: (0, security_1.clientIp)(req)
        });
        res.json(ord.rows[0]);
    }
    catch (e) {
...
router.get('/:id', (0, auth_1.requirePermission)('orders.read'), async (req, res, next) => {
    try {
        const { rows } = await pool_1.pool.query(`
      SELECT o.*, c.name AS client_name, c.code AS client_code
      FROM orders o
...
router.put('/:id', (0, auth_1.requirePermission)('orders.update'), async (req, res, next) => {
    try {
...
router.delete('/:id', (0, auth_1.requirePermission)('orders.delete'), async (req, res, next) => {
    try {
        await pool_1.pool.query(`UPDATE orders SET deleted_at = NOW() WHERE id = $1`, [req.params.id]);
        await (0, security_1.auditLog)({
...
router.post('/:id/cancel', (0, auth_1.requirePermission)('orders.cancel'), async (req, res, next) => {
    try {
        await pool_1.pool.query(`UPDATE orders SET status = 'cancelled', updated_at = NOW() WHERE id = $1`, [req.params.id]);
        await (0, security_1.auditLog)({
...
// SET parser
router.post('/set/parse', (0, auth_1.requirePermission)('orders.read'), async (req, res, next) => {
    try {
        const { codes } = req.body || {};
        if (!Array.isArray(codes))
            throw (0, types_1.BadRequest)('codes massivi kerak');
        const RX = /^SET(\d+)-([A-Z0-9]+)-([A-Z0-9]+)-([A-Z0-9]+)$/i;
        const parsed = codes.map((c) => {
            const t = String(c || '').trim().toUpperCase();
            const m = t.match(RX);
            if (!m)
                return { raw: c, ok: false, error: 'Format noto\'g\'ri' };
...
router.post('/set/create-order', (0, auth_1.requirePermission)('orders.create'), async (req, res, next) => {
    try {
        const { client_id, codes, deadline, notes } = req.body || {};
        if (!client_id)
            throw (0, types_1.BadRequest)('client_id kerak');
        ...
        return await (0, pool_1.withTransaction)(async (client) => {
            ...
            if (!counts.size)
                throw (0, types_1.BadRequest)('Hech qaysi SET kod to\'g\'ri formatda emas');
            ...
            for (const [, g] of counts) {
                ...
                if (!model.rows.length || !color.rows.length || !size.rows.length) {
                    throw (0, types_1.BadRequest)(`${g.model_code}/${g.color_code}/${g.size_code}: model yoki rang yoki size topilmadi (master-data ga qo'shing)`);
                }
                items.push({
...
router.post('/speka/parse', (0, auth_1.requirePermission)('orders.read'), async (req, res, next) => {
    try {
        const { text } = req.body || {};
        if (!text || typeof text !== 'string')
            throw (0, types_1.BadRequest)('text kerak');
        ...
        const total_qty = items.reduce((s, b) => s + Object.values(b.sizes).reduce((a, q) => a + q, 0), 0);
...
router.post('/speka/create-order', (0, auth_1.requirePermission)('orders.create'), async (req, res, next) => {
    try {
        const { client_id, items, deadline, notes } = req.body || {};
        if (!client_id)
            throw (0, types_1.BadRequest)('client_id kerak');
        if (!Array.isArray(items) || !items.length)
            throw (0, types_1.BadRequest)('items kerak');
        return await (0, pool_1.withTransaction)(async (client) => {
            const ord = await client.query(`
        INSERT INTO orders (client_id, order_type, status, deadline, notes, created_by, external_code)
      ...
      `, [client_id, deadline || null, notes || null, req.user.id]);
            ...
            for (const item of items) {
                ...
                if (!model.rows.length) {
                    throw (0, types_1.BadRequest)(`Model "${item.model_code}" topilmadi`);
                }
                for (const [sizeCode, qty] of Object.entries(item.sizes || {})) {
                    if (!qty || qty <= 0)
                        continue;
                    const size = await client.query(`SELECT id FROM sizes WHERE code = $1`, [sizeCode]);
                    if (!size.rows.length) {
                        throw (0, types_1.BadRequest)(`Size "${sizeCode}" topilmadi`);
                    }
                    await client.query(`
            INSERT INTO order_items (order_id, model_id, color_id, size_id, ordered_qty)
            VALUES ($1, $2, $3, $4, $5)
...
router.post('/parse-set', (0, auth_1.requirePermission)('orders.create'), async (req, res, next) => {
    try {
        const { codes } = req.body || {};
        if (!Array.isArray(codes) || !codes.length) {
            throw (0, types_1.BadRequest)('codes: string[] kerak');
        }
        const parsed = await (0, orders_parsers_1.parseSetCodes)(codes);
        res.json({ parsed });
    }
    catch (e) {
        next(e);
...
router.post('/speka', (0, auth_1.requirePermission)('orders.create'), async (req, res, next) => {
    try {
        const { client_id, deadline, priority, speka_number, items, notes } = req.body || {};
        if (!client_id)
            throw (0, types_1.BadRequest)('client_id kerak');
        const parsed = await (0, orders_parsers_1.parseSpekaOrder)({ speka_number, items });
        await pool_1.pool.query('BEGIN');
        try {
            // Create the order
            const ord = await pool_1.pool.query(`
...
router.post('/from-sets', (0, auth_1.requirePermission)('orders.create'), async (req, res, next) => {
    try {
        const { client_id, deadline, priority, codes, notes } = req.body || {};
        if (!client_id)
            throw (0, types_1.BadRequest)('client_id kerak');
        if (!Array.isArray(codes) || !codes.length)
            throw (0, types_1.BadRequest)('codes[] kerak');
        const parsed = await (0, orders_parsers_1.parseSetCodes)(codes);
        if (!parsed.length)
            throw (0, types_1.BadRequest)("Hech bir SET kod parse qilinmadi");
      ...
      `, [externalCode, client_id, deadline || null, priority || 0, notes || null, req.user.id]);
            ...
            for (const p of parsed) {
                const m = await pool_1.pool.query(`SELECT id FROM models WHERE UPPER(code) = $1`, [p.model_code]);
                const c = await pool_1.pool.query(`SELECT id FROM colors WHERE UPPER(code) = $1 OR UPPER(name_uz) = $1`, [p.color_code]);
                const s = await pool_1.pool.query(`SELECT id FROM sizes WHERE UPPER(code) = $1`, [p.size_code]);
