...
router.post('/qr-codes', (0, auth_1.requirePermission)('production.qr.create'), async (req, res, next) => {
    try {
        const { order_id, order_item_id, model_id, color_id, size_id, quantity, qr_code } = req.body || {};
        if (!order_id || !quantity)
            throw (0, types_1.BadRequest)('order_id va quantity kerak');
        if (!Number.isInteger(quantity) || quantity < 1)
            throw (0, types_1.BadRequest)("quantity musbat butun son");
        // Generate a QR code value if not provided: BILLUR-<orderCode>-<rand>
        let code = qr_code;
        if (!code) {
            const ord = await pool_1.pool.query(`SELECT external_code FROM orders WHERE id = $1`, [order_id]);
...
router.post('/qr-codes/bulk', (0, auth_1.requirePermission)('production.qr.create'), async (req, res, next) => {
    try {
        const { order_id, mode = 'per_item' } = req.body || {};
        if (!order_id)
            throw (0, types_1.BadRequest)('order_id kerak');
        const items = await pool_1.pool.query(`
      SELECT id, model_id, color_id, size_id, quantity
      FROM order_items WHERE order_id = $1
    `, [order_id]);
        const ordRes = await pool_1.pool.query(`SELECT external_code FROM orders WHERE id = $1`, [order_id]);
...
router.get('/qr-codes', (0, auth_1.requirePermission)('production.qr.create'), async (req, res, next) => {
    try {
        ...
        const q = (0, query_1.getOptionalQueryString)(req.query.q);
        const params = [];
        const conds = [];
        if (order_id) {
            params.push(order_id);
            conds.push(`pqr.order_id = $${params.length}`);
        }
        if (stage) {
            params.push(stage);
            conds.push(`pqr.current_stage = $${params.length}`);
        }
        if (status) {
            params.push(status);
            conds.push(`pqr.status = $${params.length}`);
        }
        if (q) {
            params.push(`%${q}%`);
            conds.push(`pqr.qr_code ILIKE $${params.length}`);
        }
...
router.get('/qr-codes/:qrCode', (0, auth_1.requirePermission)('production.qr.scan'), async (req, res, next) => {
    try {
        const r = await pool_1.pool.query(`
...
router.post('/scan', (0, auth_1.requirePermission)('production.qr.scan'), async (req, res, next) => {
    try {
        const result = await (0, scanning_service_1.recordScan)({
        ...
        // Broadcast to SSE subscribers (realtime dashboard)
        (0, sse_router_1.publishEvent)('scan', {
            action: result.action,
...
router.post('/qr-codes/:id/override', (0, auth_1.requirePermission)('production.qr.override'), async (req, res, next) => {
    try {
        const { reason } = req.body || {};
        if (!reason || reason.length < 5)
            throw (0, types_1.BadRequest)("'reason' kerak (kamida 5 belgi)");
        const r = await (0, scanning_service_1.overrideScan)({
...
router.get('/qr-codes/:id/trace', (0, auth_1.requirePermission)('production.trace.view'), async (req, res, next) => {
    try {
        const qr = await pool_1.pool.query(`
    ...
    `, [req.params.id]);
        if (!qr.rows.length)
            throw (0, types_1.NotFound)();
        const scans = await pool_1.pool.query(`
    ...
    `, [qr.rows[0].id]);
        const quality = await pool_1.pool.query(`
...
router.get('/trace/by-box/:boxUid', (0, auth_1.requirePermission)('production.trace.view'), async (req, res, next) => {
    try {
        // boxes can hold multiple items; we return all QR codes that are in this box
        const box = await pool_1.pool.query(`SELECT * FROM boxes WHERE uid = $1`, [req.params.boxUid]);
        if (!box.rows.length)
            throw (0, types_1.NotFound)();
        const scans = await pool_1.pool.query(`
    ...
    `, [req.params.boxUid]);
        const qrCodes = box.rows[0].order_id ? (await pool_1.pool.query(`
...
router.get('/scans/suspicious', (0, auth_1.requirePermission)('production.qr.override'), async (req, res, next) => {
    try {
        const { rows } = await pool_1.pool.query(`
      SELECT pss.*, w.full_name AS worker_name, w.employee_code,
...
router.get('/scans/active', (0, auth_1.requirePermission)('production.qr.scan'), async (req, res, next) => {
    try {
        ...
        if (worker_id) {
            ...
            where += ` AND pss.worker_id = $${params.length}`;
        }
        const { rows } = await pool_1.pool.query(`
...
//# sourceMappingURL=scanning.router.js.map