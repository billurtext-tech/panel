...
router.get('/movements', (0, auth_1.requirePermission)('inventory.read'), async (req, res, next) => {
    try {
        const warehouse_id = (0, query_1.getOptionalQueryString)(req.query.warehouse_id);
        const model_id = (0, query_1.getOptionalQueryString)(req.query.model_id);
        const raw_material_id = (0, query_1.getOptionalQueryString)(req.query.raw_material_id);
        ...
        if (warehouse_id) {
            ...
            conds.push(`im.warehouse_id = $${params.length}`);
        }
        if (model_id) {
            params.push(model_id);
            conds.push(`im.model_id = $${params.length}`);
        }
        if (raw_material_id) {
            params.push(raw_material_id);
            conds.push(`im.raw_material_id = $${params.length}`);
        }
        if (type) {
            params.push(type);
            conds.push(`im.movement_type = $${params.length}`);
        }
        if (since) {
            params.push(since);
            conds.push(`im.occurred_at >= $${params.length}`);
        }
...
router.post('/movements', (0, auth_1.requirePermission)('inventory.move'), async (req, res, next) => {
    try {
        const { warehouse_id, movement_type, qty, model_id, color_id, size_id, raw_material_id, from_warehouse, to_warehouse, reference_type, reference_id, box_uid, worker_id, notes } = req.body || {};
        if (!warehouse_id)
            throw (0, types_1.BadRequest)('warehouse_id kerak');
        if (!movement_type || !VALID_MOVEMENT_TYPES.includes(movement_type)) {
            throw (0, types_1.BadRequest)(`movement_type: ${VALID_MOVEMENT_TYPES.join(', ')}`);
        }
        if (!Number.isFinite(qty) || qty === 0) {
            throw (0, types_1.BadRequest)("qty 0 dan farq qilishi kerak");
        }
        const hasGood = model_id && color_id && size_id;
        const hasMaterial = raw_material_id;
        if (!hasGood && !hasMaterial) {
            throw (0, types_1.BadRequest)('Yo (model_id, color_id, size_id) yoki raw_material_id kerak');
        }
        if (hasGood && hasMaterial) {
            throw (0, types_1.BadRequest)("Bir vaqtda ham mahsulot ham xom ashyo bo'lmaydi");
        }
        if (movement_type === 'transfer' && (!from_warehouse || !to_warehouse)) {
            throw (0, types_1.BadRequest)("Transfer uchun from_warehouse va to_warehouse kerak");
        }
        const result = await (0, pool_1.withTransaction)(async (client) => {
            const wh = await client.query(`SELECT 1 FROM warehouses WHERE id = $1 AND is_active`, [warehouse_id]);
            if (!wh.rows.length)
                throw (0, types_1.BadRequest)("Noto'g'ri ombor");
            if (raw_material_id) {
                const mat = await client.query(`SELECT id, current_stock FROM raw_materials WHERE id = $1 AND is_active FOR UPDATE`, [raw_material_id]);
                if (!mat.rows.length)
                    throw (0, types_1.NotFound)('Xom ashyo topilmadi');
                let signedQty = qty;
                if (movement_type === 'receipt' || movement_type === 'return') {
                    if (qty < 0)
                        throw (0, types_1.BadRequest)('Receipt/return uchun qty musbat bolishi kerak');
                    signedQty = qty;
                }
                else if (movement_type === 'issue') {
                    if (qty > 0)
                        throw (0, types_1.BadRequest)('Issue uchun qty manfiy bolishi kerak');
                    signedQty = qty;
                }
                const newStock = Number(mat.rows[0].current_stock) + signedQty;
                if (newStock < 0) {
                    throw (0, types_1.BadRequest)(`Yetarli zaxira yo'q: ${mat.rows[0].current_stock} mavjud, ${Math.abs(signedQty)} kerak`);
                }
                await client.query(`UPDATE raw_materials SET current_stock = $1 WHERE id = $2`, [newStock, raw_material_id]);
            }
...
router.get('/balance/goods', (0, auth_1.requirePermission)('inventory.read'), async (req, res, next) => {
    try {
        const warehouse_id = (0, query_1.getOptionalQueryString)(req.query.warehouse_id);
        const model_id = (0, query_1.getOptionalQueryString)(req.query.model_id);
        const params = [];
        const conds = [`im.model_id IS NOT NULL`];
        if (warehouse_id) {
            params.push(warehouse_id);
            conds.push(`im.warehouse_id = $${params.length}`);
        }
...
router.get('/materials', (0, auth_1.requirePermission)('inventory.read'), async (req, res, next) => {
    try {
        const { rows } = await pool_1.pool.query(`
...
router.post('/materials', (0, auth_1.requirePermission)('inventory.move'), async (req, res, next) => {
    try {
        const { code, name, unit, current_stock, min_stock, default_price } = req.body || {};
        if (!code || !name || !unit)
            throw (0, types_1.BadRequest)("code, name, unit kerak");
        const dup = await pool_1.pool.query(`SELECT 1 FROM raw_materials WHERE code = $1`, [code]);
        if (dup.rows.length)
            throw (0, types_1.BadRequest)('Bu kod band');
        const { rows } = await pool_1.pool.query(`
...
router.get('/today', (0, auth_1.requirePermission)('inventory.read'), async (req, res, next) => {
    try {
        const { rows } = await pool_1.pool.query(`
