...
const production_events_1 = require("./production.events");
const router = (0, express_1.Router)();
router.use(auth_1.requireAuth);
// ── POST /api/production/events — manual stage advance ─────────────────────
router.post('/events', (0, auth_1.requirePermission)('production.events.create'), async (req, res, next) => {
    try {
        const { order_item_id, to_stage, qty, from_stage, worker_id, device_id, client_event_uuid, notes } = req.body || {};
        const result = await (0, pool_1.withTransaction)(async (client) => {
            return (0, production_events_1.recordProductionEvent)(client, {
                order_item_id,
                to_stage,
...
router.get('/events', (0, auth_1.requirePermission)('production.read'), async (req, res, next) => {
    try {
        ...
        if (order_id) {
            ...
            conds.push(`pe.order_id = $${params.length}`);
        }
        if (order_item_id) {
            params.push(order_item_id);
            conds.push(`pe.order_item_id = $${params.length}`);
        }
        if (worker_id) {
        ...
        if (stage) {
            params.push(stage);
            conds.push(`pe.to_stage = $${params.length}`);
        }
        if (since) {
            params.push(since);
...
router.get('/orders/:id/progress', (0, auth_1.requirePermission)('production.read'), async (req, res, next) => {
    try {
    ...
    `, [req.params.id]);
        ...
        const disc = await pool_1.pool.query(`
...
router.get('/stages/:stage/queue', (0, auth_1.requirePermission)('production.read'), async (req, res, next) => {
    try {
        const stage = req.params.stage;
        const col = production_events_1.STAGE_QTY_COLUMN[stage];
        if (!col)
            throw (0, types_1.BadRequest)("Bu bosqich uchun queue ko'rsatilmaydi");
        const FLOW = ['cutting', 'printing', 'sewing', 'quality', 'ironing', 'packing', 'boxing', 'shipped'];
        const idx = FLOW.indexOf(stage);
        const nextCol = idx >= 0 && idx < FLOW.length - 1 ? production_events_1.STAGE_QTY_COLUMN[FLOW[idx + 1]] : null;
...
router.get('/today', (0, auth_1.requirePermission)('production.read'), async (req, res, next) => {
    try {
        const { rows } = await pool_1.pool.query(`
