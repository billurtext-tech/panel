...
router.get('/', (0, auth_1.requirePermission)('workers.read'), async (req, res, next) => {
    try {
        const stage = (0, query_1.getOptionalQueryString)(req.query.stage);
        const position = (0, query_1.getOptionalQueryString)(req.query.position);
        const active = (0, query_1.getOptionalQueryString)(req.query.active);
        const params = [];
        const conds = [`w.deleted_at IS NULL`];
        if (stage) {
            params.push(stage);
            conds.push(`w.default_stage = $${params.length}`);
        }
        if (position) {
            params.push(position);
            conds.push(`w.position = $${params.length}`);
        }
        if (active === 'true')
            conds.push(`w.is_active = true`);
        if (active === 'false')
            conds.push(`w.is_active = false`);
...
router.post('/', (0, auth_1.requirePermission)('workers.create'), async (req, res, next) => {
    try {
        const { employee_code, full_name, phone, position, default_stage, hire_date, photo_url, notes } = req.body || {};
        if (!employee_code || !full_name || !position) {
            throw (0, types_1.BadRequest)('Tabel raqami, F.I.O. va lavozim kerak');
        }
        if (!/^[A-Za-z0-9_-]{2,32}$/.test(employee_code)) {
            throw (0, types_1.BadRequest)('Tabel raqami: 2-32 belgi, A-Z 0-9 _ -');
        }
        if (!VALID_POSITIONS.includes(position)) {
            throw (0, types_1.BadRequest)(`Noto'g'ri lavozim. Ruxsat etilgan: ${VALID_POSITIONS.join(', ')}`);
        }
        if (default_stage) {
            const stg = await pool_1.pool.query(`SELECT 1 FROM production_stages WHERE id = $1 AND is_active`, [default_stage]);
            if (!stg.rows.length)
                throw (0, types_1.BadRequest)("Noto'g'ri bosqich");
        }
        const dup = await pool_1.pool.query(`SELECT 1 FROM workers WHERE employee_code = $1`, [employee_code]);
        if (dup.rows.length)
            throw (0, types_1.Conflict)('Bu tabel raqami band');
        const { rows } = await pool_1.pool.query(`
...
router.get('/:id', (0, auth_1.requirePermission)('workers.read'), async (req, res, next) => {
    try {
        const { rows } = await pool_1.pool.query(`
...
router.put('/:id', (0, auth_1.requirePermission)('workers.update'), async (req, res, next) => {
    try {
        const { full_name, phone, position, default_stage, hire_date, photo_url, notes, is_active } = req.body || {};
        const sel = await pool_1.pool.query(`SELECT * FROM workers WHERE id = $1 AND deleted_at IS NULL`, [req.params.id]);
        if (!sel.rows.length)
            throw (0, types_1.NotFound)();
...
router.delete('/:id', (0, auth_1.requirePermission)('workers.delete'), async (req, res, next) => {
    try {
        const sel = await pool_1.pool.query(`SELECT 1 FROM workers WHERE id = $1 AND deleted_at IS NULL`, [req.params.id]);
        if (!sel.rows.length)
            throw (0, types_1.NotFound)();
        await pool_1.pool.query('BEGIN');
        // Soft delete worker
        await pool_1.pool.query(`UPDATE workers SET deleted_at = NOW(), is_active = false WHERE id = $1`, [req.params.id]);
        // Revoke all QR tokens — old badges are now dead
        await pool_1.pool.query(`UPDATE worker_qr_tokens SET is_active = false WHERE worker_id = $1`, [req.params.id]);
...
router.get('/:id/scans', (0, auth_1.requirePermission)('workers.read'), async (req, res, next) => {
    try {
        const limit = Math.min(parseInt((0, query_1.getQueryString)(req.query.limit) || '50', 10) || 50, 200);
        const { rows } = await pool_1.pool.query(`
...
router.get('/:id/qr', (0, auth_1.requirePermission)('workers.read'), async (req, res, next) => {
    try {
        const { rows } = await pool_1.pool.query(`
...
//# sourceMappingURL=workers.router.js.map