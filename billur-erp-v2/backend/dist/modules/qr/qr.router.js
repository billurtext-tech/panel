...
const production_events_1 = require("../production/production.events");
const router = (0, express_1.Router)();
router.use(auth_1.requireAuth);
const TOKEN_TTL_DAYS = 365;
const DUPLICATE_WINDOW_SECONDS = 10;
// ── Generate / regenerate a worker's QR token ──────────────────────────────
router.post('/generate/:workerId', (0, auth_1.requirePermission)('qr.generate'), async (req, res, next) => {
    try {
        const w = await pool_1.pool.query(`SELECT id, full_name, employee_code FROM workers
       WHERE id = $1 AND deleted_at IS NULL AND is_active = true`, [req.params.workerId]);
        if (!w.rows.length)
            throw (0, types_1.NotFound)('Ishchi topilmadi yoki aktiv emas');
        const token = (0, crypto_1.generateQrToken)(req.params.workerId);
        const expiresAt = new Date(Date.now() + TOKEN_TTL_DAYS * 86400 * 1000);
        await (0, pool_1.withTransaction)(async (client) => {
            await client.query(`UPDATE worker_qr_tokens SET is_active = false WHERE worker_id = $1 AND is_active = true`, [req.params.workerId]);
            await client.query(`INSERT INTO worker_qr_tokens (worker_id, token, expires_at, is_active)
         VALUES ($1, $2, $3, true)`, [req.params.workerId, token, expiresAt]);
        });
...
router.get('/worker/:workerId/active', (0, auth_1.requirePermission)('qr.generate'), async (req, res, next) => {
    try {
    ...
    `, [req.params.workerId]);
        if (!r.rows.length)
            throw (0, types_1.NotFound)('Aktiv QR token yo\'q. Avval generate qiling.');
        const dataUrl = await qrcode_1.default.toDataURL(r.rows[0].token, {
            errorCorrectionLevel: 'H',
            width: 300,
            margin: 2,
        });
...
router.get('/worker/:workerId/png', (0, auth_1.requirePermission)('qr.generate'), async (req, res, next) => {
    try {
        const r = await pool_1.pool.query(`
    ...
    `, [req.params.workerId]);
        if (!r.rows.length)
            throw (0, types_1.NotFound)('Aktiv QR token yo\'q');
        const buf = await qrcode_1.default.toBuffer(r.rows[0].token, {
            errorCorrectionLevel: 'H',
            width: 600,
            margin: 4,
        });
...
router.post('/lookup', (0, security_1.rateLimit)(60, 60_000), (0, auth_1.requirePermission)('qr.scan'), async (req, res, next) => {
    try {
        const { token } = req.body || {};
        if (!token || typeof token !== 'string')
            throw (0, types_1.BadRequest)('Token kerak');
        const v = (0, crypto_1.validateQrToken)(token);
        if (!v.valid)
            throw (0, types_1.BadRequest)(`QR yaroqsiz: ${v.error}`);
        const t = await pool_1.pool.query(`SELECT t.is_active, t.expires_at,
       ...
       WHERE t.token = $1`, [token]);
        if (!t.rows.length)
            throw (0, types_1.BadRequest)("QR ro'yxatda yo'q");
        if (t.rows[0].deleted_at)
            throw (0, types_1.Forbidden)("Ishchi o'chirilgan");
        if (!t.rows[0].worker_active)
            throw (0, types_1.Forbidden)('Ishchi faol emas');
        if (!t.rows[0].is_active)
            throw (0, types_1.Forbidden)('QR bekor qilingan');
        if (t.rows[0].expires_at && new Date(t.rows[0].expires_at) < new Date()) {
            throw (0, types_1.Forbidden)('QR muddati tugagan');
        }
        res.json({
...
router.post('/scan', (0, security_1.rateLimit)(120, 60_000), (0, auth_1.requirePermission)('qr.scan'), async (req, res, next) => {
    try {
        ...
        if (!validScanTypes.includes(sType)) {
            throw (0, types_1.BadRequest)(`Noto'g'ri scan_type. Ruxsat: ${validScanTypes.join(', ')}`);
        }
        if (sType === 'stage_advance') {
            if (!stage)
                throw (0, types_1.BadRequest)('Bosqich kerak');
            if (!order_item_id)
                throw (0, types_1.BadRequest)('Mahsulot satri (order_item_id) kerak');
            if (!Number.isFinite(qty) || qty < 1 || qty > 100_000) {
                throw (0, types_1.BadRequest)("qty 1-100000 oraliqda bo'lishi kerak");
            }
        }
        // Cryptographic check
        const v = (0, crypto_1.validateQrToken)(token);
        ...
        const t = await pool_1.pool.query(`SELECT t.is_active AS token_active, t.expires_at,
       ...
       WHERE t.token = $1`, [token]);
        ...
        if (stage) {
            const stg = await pool_1.pool.query(`SELECT 1 FROM production_stages WHERE id = $1`, [stage]);
            if (!stg.rows.length)
                throw (0, types_1.BadRequest)("Noto'g'ri bosqich");
        }
        if (order_item_id) {
            const oi = await pool_1.pool.query(`SELECT 1 FROM order_items WHERE id = $1`, [order_item_id]);
            if (!oi.rows.length)
                throw (0, types_1.BadRequest)('Mahsulot satri topilmadi');
        }
        // Suspicious detection (unchanged from Phase 2)
        let isSuspicious = false;
        let reason = null;
        if (sType === 'stage_advance') {
            const dup = await pool_1.pool.query(`
      ...
      `, [
                ...
                isSuspicious, reason,
                (0, security_1.clientIp)(req), req.headers['user-agent']?.toString().slice(0, 500) || null
            ]);
            let eventResult = null;
            if (sType === 'stage_advance' && !isSuspicious) {
                eventResult = await (0, production_events_1.recordProductionEvent)(client, {
                    order_item_id,
                    to_stage: stage,
                    qty,
                    worker_id: workerId,
...
// ── List scans ─────────────────────────────────────────────────────────────
router.get('/scans', (0, auth_1.requirePermission)('qr.scan'), async (req, res, next) => {
    try {
        ...
        const suspicious = (0, query_1.getOptionalQueryString)(req.query.suspicious);
        const since = (0, query_1.getOptionalQueryString)(req.query.since);
        const params = [];
        const conds = [];
        if (worker_id) {
            params.push(worker_id);
            conds.push(`s.worker_id = $${params.length}`);
        }
        if (stage) {
            params.push(stage);
            conds.push(`s.stage = $${params.length}`);
        }
        if (suspicious === 'true')
            conds.push(`s.is_suspicious = true`);
        if (suspicious === 'false')
            conds.push(`s.is_suspicious = false`);
        if (since) {
            params.push(since);
            conds.push(`s.scanned_at >= $${params.length}`);
        }
...
router.post('/scans/:id/approve', (0, auth_1.requirePermission)('qr.approve'), async (req, res, next) => {
    try {
        const result = await (0, pool_1.withTransaction)(async (client) => {
            const scn = await client.query(`SELECT * FROM qr_scans WHERE id = $1 FOR UPDATE`, [req.params.id]);
            if (!scn.rows.length)
                throw (0, types_1.NotFound)('Scan topilmadi');
            const s = scn.rows[0];
            if (!s.is_suspicious)
                throw (0, types_1.NotFound)('Bu scan shubhali emas');
            if (s.approved_by)
                throw (0, types_1.NotFound)('Allaqachon tasdiqlangan');
            await client.query(`
        UPDATE qr_scans
