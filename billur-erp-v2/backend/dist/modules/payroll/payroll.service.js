...
exports.calculateAllPayroll = calculateAllPayroll;
const pool_1 = require("../../shared/database/pool");
const pool_2 = require("../../shared/database/pool");
const types_1 = require("../../shared/types");
async function calculatePayroll(opts) {
    const { worker_id, period_start, period_end } = opts;
    if (!worker_id || !period_start || !period_end) {
        throw (0, types_1.BadRequest)('worker_id, period_start, period_end kerak');
    }
    return (0, pool_1.withTransaction)(async (client) => {
        // Verify worker
        const wr = await client.query(`SELECT id, full_name FROM workers WHERE id = $1 AND deleted_at IS NULL`, [worker_id]);
        if (!wr.rows.length)
...
async function calculateAllPayroll(opts) {
    // Find all workers who had finished scans in the period
    const workers = await pool_2.pool.query(`
