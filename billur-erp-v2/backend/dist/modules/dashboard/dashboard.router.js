...
router.get('/overview', async (req, res, next) => {
    try {
      ...
      `),
            pool_1.pool.query(`SELECT COUNT(*)::int AS c FROM clients WHERE deleted_at IS NULL AND is_active`),
            pool_1.pool.query(`SELECT COUNT(*)::int AS c FROM workers WHERE deleted_at IS NULL AND is_active`),
            pool_1.pool.query(`SELECT COUNT(*)::int AS c FROM discrepancies WHERE status = 'open'`),
            pool_1.pool.query(`
