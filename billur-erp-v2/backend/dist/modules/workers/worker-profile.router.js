...
async function workerForUser(userId) {
    const r = await pool_1.pool.query(`SELECT id FROM workers WHERE user_id = $1`, [userId]);
    return r.rows[0]?.id;
}
function canSeeWorker(req, workerUserId) {
    if (req.user.permissions.includes('workers.view_all'))
        return true;
...
router.get('/me/stats', async (req, res, next) => {
    try {
        ...
        if (!wid)
            return res.json({ today: 0, week: 0, month: 0, models: [], attendance: null });
...
router.get('/me/productivity', async (req, res, next) => {
    try {
        const wid = await workerForUser(req.user.id);
        if (!wid)
            return res.json({ items: [], total_quantity: 0, total_scans: 0 });
...
router.get('/me/scans', async (req, res, next) => {
    try {
        const wid = await workerForUser(req.user.id);
        if (!wid)
            return res.json([]);
...
router.get('/:id/profile', async (req, res, next) => {
    ...
    `, [req.params.id]);
        ...
        if (!canSeeWorker(req, r.rows[0].user_id))
...
router.put('/:id/profile', async (req, res, next) => {
    try {
        const sel = await pool_1.pool.query(`SELECT user_id FROM workers WHERE id = $1`, [req.params.id]);
        if (!sel.rows.length)
            throw (0, types_1.NotFound)();
        ...
        if (!updates.length)
            return res.json({ ok: true, message: 'No-op' });
    ...
    `, params);
        await (0, security_1.auditLog)({
            ...
            action: 'update', after_value: req.body, ip_address: (0, security_1.clientIp)(req),
        });
        res.json(r.rows[0]);
    }
    catch (e) {
...
router.post('/:id/documents', (0, auth_1.requirePermission)('workers.documents.upload'), async (req, res, next) => {
    try {
        const { document_type, file_name, file_path, file_size, mime_type, issued_date, expiry_date, notes } = req.body || {};
        if (!document_type || !file_name || !file_path) {
            throw (0, types_1.BadRequest)('document_type, file_name, file_path kerak');
        }
        const r = await pool_1.pool.query(`
...
router.post('/:id/documents/upload', (0, auth_1.requirePermission)('workers.documents.upload'), (req, res, next) => {
    (0, upload_1.uploadDocument)(req, res, async (err) => {
        if (err) {
            const message = err instanceof Error ? err.message : 'Upload xato';
            return next((0, types_1.BadRequest)(message));
        }
        const file = req.file;
        if (!file)
            return next((0, types_1.BadRequest)('Fayl tanlanmadi'));
        try {
            const { document_type, issued_date, expiry_date, notes } = req.body || {};
            if (!document_type) {
                return next((0, types_1.BadRequest)('document_type kerak'));
            }
            const r = await pool_1.pool.query(`
          INSERT INTO worker_documents
            (worker_id, document_type, file_name, file_path, file_size, mime_type,
        ...
        `, [
                ...
                file.originalname, (0, upload_1.fileUrl)(file.path),
            ...
            await (0, security_1.auditLog)({
                ...
                action: 'upload', metadata: {
                    worker_id: req.params.id, document_type, file_size: file.size,
                },
                ip_address: (0, security_1.clientIp)(req),
            });
            res.json(r.rows[0]);
...
router.delete('/:workerId/documents/:docId', (0, auth_1.requirePermission)('workers.documents.view_all'), async (req, res, next) => {
    try {
        const r = await pool_1.pool.query(`
    ...
    `, [req.params.docId, req.params.workerId]);
        ...
        await (0, security_1.auditLog)({
            ...
            action: 'delete', before_value: r.rows[0], ip_address: (0, security_1.clientIp)(req),
        });
        res.json({ ok: true });
    }
    catch (e) {
...
//# sourceMappingURL=worker-profile.router.js.map